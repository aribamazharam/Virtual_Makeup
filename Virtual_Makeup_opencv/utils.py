...
# Based on input facial features create mask w.r.to colors
def add_mask(mask: np.array, idx_to_coordinates: dict, face_connections: list, colors: list):
    ...
    for i, connection in enumerate(face_connections):
        # Extract (x,y) w.r.to image for each coordinates
...
# Detect the average color of the lips
def detect_lip_color(image: np.array, lip_points: list, landmarks: dict):
    lip_pixels = []
    for idx in lip_points:
        if idx in landmarks:
            x, y = landmarks[idx]
            lip_pixels.append(image[y, x])

    if not lip_pixels:
        return np.array([0, 0, 0])

    lip_pixels = np.array(lip_pixels)
    average_color = np.mean(lip_pixels, axis=0)
    return average_color.astype(np.uint8)

# Mix two colors with a given ratio
def mix_colors(color1, color2, ratio=0.5):
    mixed_color = (1 - ratio) * color1 + ratio * color2
    return mixed_color.astype(np.uint8)
